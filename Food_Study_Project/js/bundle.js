...
var calc = function calc() {
  ...
  if (localStorage.getItem('sex')) {
    //проверяем, записан ли заранее пол. Если нет, то берем женский по умолчанию.
  ...
  function initLocalSettings(selector, activeClass) {
    // функция для того, чтобы после загрузки страницы были выделены блоки, которые записаны в хранилище
    var elements = document.querySelectorAll(selector);
    elements.forEach(function (elem) {
      elem.classList.remove(activeClass);
      if (elem.getAttribute('id') === localStorage.getItem('sex')) {
        //если при переборе элементов значение id совпадет с тем, что ледит в хранилище
  ...
  function calcTotal() {
    ...
    if (!sex || !height || !weight || !age || !ratio) {
      // если что-то не заполнено, то расчитывать не будем
      result.textContent = 'WTF';
      return;
    }
    if (sex === 'female') {
      // если для женщины, формула одна, для мужчины другая
  ...
  function getStaticInformation(selector, activeClass) {
    //функция для сбора данных, из незаполняемых (выбранных) элементов
    var elements = document.querySelectorAll(selector); //получаем все div из элемента parrentSelector

    elements.forEach(function (elem) {
      elem.addEventListener('click', function (e) {
        // при нажатии на один из елементов
        if (e.target.getAttribute('data-ratio')) {
          // проверяем есть ли у него аттрибут data-ratio
          ...
          // если это не элемент активности, то элемент пола, а у них вместо data-ratio мы прописали id для их разделения
  ...
  function getDynamicInformation(selector) {
    // функция для сбора данных, из заполняемых полей
    ...
    input.addEventListener('input', function () {
      ...
      switch (input.getAttribute('id')) {
        // когда в одно из полей будем что-то вводить, код будет понимать в какое поле вводим по id
        case 'height':
          height = +input.value;
          break;
        case 'weight':
...
var cards = function cards() {
  var MenuCard = /*#__PURE__*/function () {
    function MenuCard(src, alt, tittle, descr, price, parrentSelector) {
      _classCallCheck(this, MenuCard);
      //Допустим хотим будущей карточке присвоить неизвестное количество дополнительных классов в будущем, используем rest оператор
      this.src = src;
      this.alt = alt;
      this.tittle = tittle;
      this.descr = descr;
    ...
    return _createClass(MenuCard, [{
      ...
      value: function changeToRub() {
        // метод для конвертации валюты в рубль
        this.price *= this.transfer;
      }
    }, {
      key: "render",
      value: function render() {
        //для добавления новой карточки на страницу в структуру html
        var element = document.createElement('div');
        if (this.classes.length === 0) {
          //если в rest не были вообще переданы аргументы, то добавляем класс, по умолчанию, без которого вообще никак, поедет верстка
          this.classes = 'menu__item';
          element.classList.add(this.classes);
        } else {
          this.classes.forEach(function (className) {
            return element.classList.add(className);
          }); // перебрали все элементы массива с классами (rest), каждый класс добавили к нашей карточке
        }
  ...
  var deleteCard = document.querySelectorAll('.menu__item'); //удаляем старые карточки
  deleteCard.forEach(function (item) {
    item.remove();
  });
  (0,_services_services__WEBPACK_IMPORTED_MODULE_14__.getResources)('http://localhost:3000/menu').then(function (data) {
...
var forms = function forms(formSelector, modalTimerId) {
  var forms = document.querySelectorAll(formSelector);
  var message = {
    // сообщения для юзера в разных ситуациях
    loading: 'img/form/spinner.svg',
    success: 'Спасибо! Скоро мы с Вами свяжемся',
    failure: 'Что-то пошло не так...'
  };
  forms.forEach(function (item) {
    // При заполнении любой формы будет происходить нижеперечисленное
    bindPostData(item);
  });
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      //срабатывает когда мы пытемся оправить какую-то форму
      ...
      var statusMessage = document.createElement('img'); // Добавляем блок с сообщением пользователю о статусе отправки данных
      ...
      var formData = new FormData(form); // Это позволяет преобразовать данные от форм ввода в привычную форму объекта
    ...
    function showThanksModal(message) {
      // новое модальное окно с благодарностью

      var prevModalDialog = document.querySelector('.modal__dialog'); //функционируем с имеющимся модальным окном
      prevModalDialog.classList.add('hide'); //временно скрываем его, чтобы не удалять и не создавать заново при новой необходимости
      (0,_modal__WEBPACK_IMPORTED_MODULE_10__.openModal)('.modal', modalTimerId); //создавали ранее, открывает модальное окно
...
var modal = function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalTrigger.forEach(function (btn) {
    ...
    }); //коллбек функция прописывается, потому что это обработчик событий и иначе она работать не будет
  });

  /*     modalCloseBtn.addEventListener('click', closeModal); */

  modal.addEventListener('click', function (e) {
    //если кликнуть мимо модального окна, то оно тоже закроется
    if (e.target === modal || e.target.getAttribute('data-close') == '') {
      closeModal(modalSelector);
    }
  });
  document.addEventListener('keydown', function (e) {
    // Если нажали Esc, окно тоже закрывается. e.code - кодовое название клавиш, надо их гуглить
    if (e.code === 'Escape' && modal.classList.contains('show')) {
      //проверка также на то, открыто ли сейчас окно, иначе Esc будет его сам открывать
      closeModal(modalSelector);
    }
  });
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
      // проверяет докрутили ли страницу до конца
...
var slider = function slider(_ref) {
  ...
  var offset = 0;
  if (slides.length < 10) {
    //чтобы индекс прописывался с 0 в начале, например 03, 07 и тд
  ...
  slides.forEach(function (slide) {
    //каждый слайд остается с родной шириной, не растягивается на всю карусель - 400%
    slide.style.width = width;
  });
  slider.style.position = 'relative';
  var indicators = document.createElement('ol'); //создаем список(ol - order-list) из точек навигации

  var dots = []; //создаем массив для точек
  ...
  for (var i = 0; i < slides.length; i++) {
    var dot = document.createElement('li'); //создаем точки по количеству слайдов (li - list-item)
  ...
  function slidesSetting() {
    ...
    }); // для изменения активного индикатора
  ...
  next.addEventListener('click', function () {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      //если текущее положение индекса конечное, то возвращаемся к начальному
  ...
  dots.forEach(function (dot) {
    // для перемещения по слайдам кликом по навигации
    dot.addEventListener('click', function (e) {
      ...
      slidesField.style.transform = "translateX(-".concat(offset, "px)"); //перемещаем на нужный слайд
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    // i=0 - значит, что если аргумент при вызове не быдет задан, то он автоматически приравняется к нулю
  ...
  tabsParent.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      //вырезали точку из tabsSelector
...
var timer = function timer(id, deadLine) {
  ...
  function getTimeRemaining(endTime) {
    ...
    var t = Date.parse(endTime) - Date.parse(new Date()); // Date.parse - преобразование даты в числовое выражение (миллисекунды). newDate - текущая дата
    ...
    return {
      'total': t,
      // актуальная разница между заявленным дедлайном и текущей датой. Если дедлайн просрочен - становится отрицательной
      'days': days,
      'hours': hours,
      'minutes': minutes,
      'seconds': seconds
  ...
  function setClock(selector, endTime) {
    ...
    var timeInterval = setInterval(updateClock, 1000); // Обновляет таймер на странице 1 раз в секунду
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _context.next = 2;
          return fetch(url, {
            // await не дает создать переменную res до тех пор, пока не выполнится запрос на сервер
            method: 'POST',
            headers: {
...
/* harmony import */ var es6_promise_polyfill__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! es6-promise-polyfill */ "./node_modules/es6-promise-polyfill/promise.js");
/* harmony import */ var es6_promise_polyfill__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(es6_promise_polyfill__WEBPACK_IMPORTED_MODULE_1__);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.timer', '2024-08-31');
